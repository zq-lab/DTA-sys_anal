...
class DAT3(nn.Module):
    ...
    def forward(self, protein, smiles):
        # 输入:
        # protein: 长度为batch_size的列表, 元素为shape=[len_seq, embedding_dim]的张量, 其中len_seq指蛋白质序列长度, emb_dim指蛋白质嵌入维度
        # smiles: 长度为batch_size的列表, 元素为shape=[len_smiles]的张量, 其中len_smiles为smiles的长度
